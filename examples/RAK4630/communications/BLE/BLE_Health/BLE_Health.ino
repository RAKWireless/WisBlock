/**
   ...
   @brief This sketch demonstrate how to get health data from Max30105
   @version 0.1
   @date 2020-07-28
...
void setup()
{
  ...
  max30105_init();
  Serial.println("RAK4631 HRM Example");
  Serial.println("-----------------------\n");
...
void loop()
{
  if ( Bluefruit.connected() ) {
    uint8_t hrmdata[2] = {0};           // Sensor connected, increment BPS value   
    ...
    detect_flag = 0;
    hrmdata[0] = 0b00000110;
    hrmdata[1] = tmp; 
    if ( hrmc.notify(hrmdata, sizeof(hrmdata))){
      Serial.print("Heart Rate Measurement updated to: "); Serial.println(tmp); 
    }else{
