...
#### How the the `loraTask` is woken up.
If the SX1262 sets its interrupt to inform the MCU about an event (can be RX, TX or error events), it is handled inside the library by it's own `loraTask`. Depending on the event, the callbacks we defined with    
```cpp  
static lmh_callback_t lora_callbacks
```    
are called.

#### Handling LoRa events
